...
#include <world/ColumnGenerator.hpp>
#include <world/TreeGenerator.hpp>
...
namespace world {
    ...
    void ChunkManager::update() {
        ...
        config->superchunk = static_cast<GLuint>(this->chunks.size());
        config->chunk = config->superchunk * SuperChunk::CHUNK_SIZE;
        config->cube = config->superchunk * SuperChunk::SIZE;
        config->face = config->cube * 6;
    }
    ...
    void ChunkManager::render() {
        ...
        glm::vec3 lightPos = glm::vec3(MVMatrix * glm::vec4(engine->sun->getPosition(), 0));
        glm::vec3 lightColor = app::Config::getLightColor(engine->tickCycle);
        if (engine->underwater) {
            lightColor *= glm::vec3(0.36, 0.56, 1);
        }
        GLfloat lightDirIntensity = app::Config::getLightDirIntensity(engine->tickCycle);
        GLfloat lightAmbIntensity = app::Config::getLightAmbIntensity(engine->tickCycle);
        ...
        config->rendered_face = 0;
        std::for_each(
            this->chunks.begin(), this->chunks.end(),
            [&config](const auto &entry) { config->rendered_face += entry.second->render(false); }
        );
        glDisable(GL_CULL_FACE);
        std::for_each(
            this->chunks.begin(), this->chunks.end(),
            [&config](const auto &entry) { config->rendered_face += entry.second->render(true); }
        );
