...
namespace world {
    ...
    bool Chunk::occluded(CubeData type, GLint x, GLint y, GLint z, CubeData direction) const {
        ...
        if (onBorder(static_cast<GLubyte>(x), static_cast<GLubyte>(y), static_cast<GLubyte>(z))) {
            ...
            if (type & ALPHA) {
                switch (direction) {
                    case CubeData::FACE:
                        return engine->chunkManager->get({ x, y, z + 1 }) != CubeData::AIR;
                    case CubeData::TOP:
                        return engine->chunkManager->get({ x, y + 1, z }) != CubeData::AIR;
                    case CubeData::BACK:
                        return engine->chunkManager->get({ x, y, z - 1 }) != CubeData::AIR;
                    case CubeData::BOTTOM:
                        return engine->chunkManager->get({ x, y - 1, z }) != CubeData::AIR;
                    case CubeData::LEFT:
                        return engine->chunkManager->get({ x - 1, y, z }) != CubeData::AIR;
                    case CubeData::RIGHT:
                        return engine->chunkManager->get({ x + 1, y, z }) != CubeData::AIR;
                    default:
            ...
            switch (direction) {
                case CubeData::FACE:
                    return !(engine->chunkManager->get({ x, y, z + 1 }) & ALPHA);
                case CubeData::TOP:
                    return !(engine->chunkManager->get({ x, y + 1, z }) & ALPHA);
                case CubeData::BACK:
                    return !(engine->chunkManager->get({ x, y, z - 1 }) & ALPHA);
                case CubeData::BOTTOM:
                    return !(engine->chunkManager->get({ x, y - 1, z }) & ALPHA);
                case CubeData::LEFT:
                    return !(engine->chunkManager->get({ x - 1, y, z }) & ALPHA);
                case CubeData::RIGHT:
                    return !(engine->chunkManager->get({ x + 1, y, z }) & ALPHA);
                default:
                    throw std::runtime_error("Received an invalid direction");
    ...
    GLuint Chunk::update() {
        ...
        glVertexAttribPointer(
            ...
            reinterpret_cast<const GLvoid *>(offsetof(world::CubeVertex, vertex))
        );
        glVertexAttribPointer(
            VERTEX_ATTR_NORMAL, 3, GL_BYTE, GL_FALSE, sizeof(world::CubeVertex),
            reinterpret_cast<const GLvoid *>(offsetof(world::CubeVertex, normal))
        );
        glVertexAttribPointer(
            VERTEX_ATTR_TEXTURE, 2, GL_BYTE, GL_FALSE, sizeof(world::CubeVertex),
            reinterpret_cast<const GLvoid *>(offsetof(world::CubeVertex, texture))
        );
        glVertexAttribIPointer(
            VERTEX_ATTR_DATA, 1, GL_SHORT, sizeof(world::CubeVertex),
            reinterpret_cast<const GLvoid *>(offsetof(world::CubeVertex, data))
        );
        glBindBuffer(GL_ARRAY_BUFFER, 0);
        glBindVertexArray(0);
        ...
        glVertexAttribIPointer(
            VERTEX_ATTR_DATA, 1, GL_UNSIGNED_SHORT, sizeof(world::CubeVertex),
            reinterpret_cast<const GLvoid *>(offsetof(world::CubeVertex, data))
        );
        glBindBuffer(GL_ARRAY_BUFFER, 0);
        glBindVertexArray(0);
